...
(
    ...
    (
        SELECT
            ...
            LUGA.HK_USER_ID,
            SAH.EVENT
        FROM DANILAKOVRIGINYANDEXRU__DWH.L_USER_GROUP_ACTIVITY LUGA 
        LEFT JOIN DANILAKOVRIGINYANDEXRU__DWH.S_AUTH_HISTORY SAH ON LUGA.HK_L_USER_GROUP_ACTIVITY = SAH.HK_L_USER_GROUP_ACTIVITY
        WHERE LUGA.HK_GROUP_ID IN (
                                   SELECT HK_GROUP_ID 
                                   FROM DANILAKOVRIGINYANDEXRU__DWH.H_GROUPS HG 
...
,USER_GROUP_MESSAGES AS (
    ...
    FROM (
        SELECT
            ...
            LUM.HK_USER_ID,
            LGD.HK_MESSAGE_ID
        FROM DANILAKOVRIGINYANDEXRU__DWH.L_GROUPS_DIALOGS LGD 
        LEFT JOIN DANILAKOVRIGINYANDEXRU__DWH.L_USER_MESSAGE LUM ON LGD.HK_MESSAGE_ID = LUM.HK_MESSAGE_ID
         ) TMP
    GROUP BY 1
)
SELECT 
